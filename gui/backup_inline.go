...
func RunBackupInline(opts BackupOptions) error {
	...
	// BUT: Skip split if previous backup exists (deduplication will make it small)
	...
	if err != nil {
		...
		if analysis.ShouldSplit {
			...
			// Check if previous backup exists - if yes, skip split (dedup makes it small)
			...
			if err == nil && len(previousDidx) > 0 {
				writeBackupLog(fmt.Sprintf("[Auto-Split] Previous backup found (%d bytes) - SKIPPING split (deduplication will optimize size)", len(previousDidx)))
				analysis.ShouldSplit = false
			} else {
				writeBackupLog("[Auto-Split] First backup detected - split will be applied")
			}
		}
